...
export const ResourcesItems = ({role, resources, onFilter, selection, onDelete, onUpdate}) => {
    ...
    return (<>
        <SearchWithDropdown types={resources_genders} search={search} onChange={e => setSearch(e.target.value)} onFilter={onFilter} >
            {role && <button onClick={() => onUpdate(false)} data-bs-target="#modal-resource" data-bs-toggle="modal" className='btn btn-primary' style={{marginLeft: "10px", width: '46px',height: '46px',padding: '0px'}}>
                            ...
                            <div className="row">
                                <div className="col-8">
                                    <div className="d-flex align-items-center">
                                        <div>
                                            <div style={{marginBottom: '-2px'}} className='text-capitalize'><span style={{fontSize: '14px',fontWeight: 'bold'}}>{resource.title}</span></div>
                                            <div><span style={{fontSize: '11px'}}>{resource.gender}</span></div>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-4 d-flex justify-content-end">
                                    {role && <div>
                                        <button onClick={() => handleDelete(resource._id)} data-bs-target="#modal-delete-resource" data-bs-toggle="modal" className="btn btn-primary btn-sm bg-body border-0" type="button" style={{padding: '0px',width: '35px',height: '35px'}}>
                                        ...
                                        <div className="dropdown-menu" style={{overflow: 'hidden'}}>
                                            <a className="dropdown-item" onClick={() => onUpdate(true, resource)} data-bs-target="#modal-resource" data-bs-toggle="modal" style={{cursor: 'pointer'}}>Update</a>
                                        </div> */}
                                    </div>}
                                </div>
                            </div>
        ...
        <DeleteResource onDelete={onDelete} resourceId={resourceId} />
    </>
    )
}
