...
import { DeleteLibrary } from '../components/elements/modal-delete-libraries';
    ...
    <div>
      ...
      <div class="row gy-3">
        {libraries.filter((library) => {
          ...
          }).map((library, i) => (<LibrariesItems deleteId={deleteId} btnFollow={false} library={library} key={i}></LibrariesItems>))}
        </div>
      </div> : 
      <div>
      ...
      <DeleteLibrary libraryId={libraryId} updateList={(newItems) => setLibraries(newItems)} />
    </div>
  )
}
